...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case baron:
        return baronCard(choice1, state);
...
/* Refractored implementation for the baron card */
    ...
    {                               //Boolean true or going to discard an estate
        ...
        while (card_not_discarded)
        {
            ...
            {                      //Found an estate card!
                state->coins += 10; //Add 4 coins to the amount of coins (bug: adds 10 instead of 4)
            ...
            else if (p > state->handCount[currentPlayer])
            {
                ...
                if (supplyCount(estate, state) > 0)
                {
                    ...
                    state->supplyCount[estate]++; //Decrement estates (bug: increments instead of decrements)
                    if (supplyCount(estate, state) == 0)
                    {
                        isGameOver(state);
                    }
...
/* Refractored implementation for the minion card */
int minionCard(int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    {
        //discard hand
        while (numHandCards(state) > 1) //(bug: changed the number of cards that the player is allowed
                                        //      keep when discarding their hand from 0 to 1)
        {
            discardCard(handPos, currentPlayer, state, 0);
        }

        //draw 4
        for (i = 0; i <= 5; i++) //(bug: changed < to <=)
        {
            drawCard(currentPlayer, state);
        }
...
/* Refractored implementation for the ambassador card */
int ambassadorCard(int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    j = 1; //used to check if player has enough cards to discard (bug: change from 0 to 1)
...
/* Refractored implementation for the tribute card */
int tributeCard(struct gameState *state)
{
    int tributeRevealedCards[2] = {-1, -1};
    int currentPlayer = whoseTurn(state);
    int nextPlayer = currentPlayer + 1; //(bug: change from + 1 to -1)
    int i;
    ...
    for (i = 0; i <= 2; i++)
    {
        ...
        { //Action Card
            state->numActions = state->numActions + 3; //(bug: change +2 to +3)
        }
    }
...
/* Refractored implementation for the mine card */
int mineCard(int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    {
        return 0; //(bug: changed return 0 to return -1)
    }
    ...
    if ((getCost(state->hand[currentPlayer][choice1]) + 6) > getCost(choice2)) //(bug: changed + 3 to + 6)
    {
        return -1;
    }
